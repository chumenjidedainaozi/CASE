...
class SA_DDI(torch.nn.Module):
    def __init__(self, in_dim, edge_in_dim, hidden_dim=64, n_iter=10, rels_nums = 963, batch_size = 2048):
        ...
        self.scale_item = torch.nn.Parameter(torch.ones(rels_nums, batch_size))
        self.rels_nums = rels_nums

        # 对比学习mlp
        self.mlp_contrastive_learning = nn.Linear(4593, hidden_dim)

    def forward(self, triples, label):
        h_data, t_data, rels, head_feature, tail_feature = triples
        # print(head_feature.shape) #(b, 4593)
        # print(tail_feature.shape) #(b, 4593)
        ...
        #对比学习 特征变化
        h_feature_final = self.mlp_contrastive_learning(head_feature)
        t_feature_final = self.mlp_contrastive_learning(tail_feature)

        h_pos = torch.sum(torch.mul(h_final, h_feature_final), 1) 
        h_neg = torch.sum(torch.mul(h_final, h_feature_final[torch.randperm(h_feature_final.size()[0])]), 1)

        t_pos = torch.sum(torch.mul(t_final, t_feature_final), 1) 
        t_neg = torch.sum(torch.mul(t_final, t_feature_final[torch.randperm(t_feature_final.size()[0])]), 1)

        h_one = torch.cuda.FloatTensor(h_neg.shape[0]).fill_(1)
        t_one = torch.cuda.FloatTensor(t_neg.shape[0]).fill_(1)
        
        #对比学习loss
        con_loss = (torch.sum(-torch.log(1e-8 + torch.sigmoid(h_pos))-torch.log(1e-8 + (h_one - torch.sigmoid(h_neg)))) + torch.sum(-torch.log(1e-8 + torch.sigmoid(t_pos))-torch.log(1e-8 + (t_one - torch.sigmoid(t_neg)))))/1000
        ...
        # chcf
        likelihood= torch.nn.functional.relu(h_final * t_final).sum(-1)
        alpha = 0.1
        loss_positive = 0
        for i in range(self.rels_nums):
            batch_positive_rels_index_rels = (rels==i)
            batch_positive_rels_index_labels = (label == 1)
            batch_positive_rels_index = torch.logical_and(batch_positive_rels_index_rels, batch_positive_rels_index_labels)
            likelihood_positive = likelihood[batch_positive_rels_index]
            criterion_positive = self.scale_item[i][batch_positive_rels_index]
            loss_positive += torch.sum(torch.pow(torch.nn.functional.relu(criterion_positive  - likelihood_positive), 2))/1000

        loss_negative = 0
        for i in range(self.rels_nums):
            batch_negative_rels_index_rels = (rels==i)
            batch_negative_rels_index_labels = (label == 0)
            batch_negative_rels_index = torch.logical_and(batch_negative_rels_index_rels, batch_negative_rels_index_labels)
            likelihood_negative = likelihood[batch_negative_rels_index]
            criterion_negative = self.scale_item[i][batch_negative_rels_index]
            loss_negative += torch.sum(torch.pow(torch.nn.functional.relu(criterion_negative  - alpha * likelihood_negative), 2))/1000
            
        loss = loss_negative + loss_positive + con_loss
